function runTask2()
    ...
    % Parameters    
    params = struct(...
        'spdDataFile', 'BVAMS_White_Guns_At_Max_HL.mat', ...           % Datafile containing the display SPDs
        'psfDataSubDir', 'FullVis_PSFs_20nm_Subject1', ...         % Subdir where the PSF data live
        'psfDataFile', '',...                                       % Datafile containing the PSF data
        'letterSizesNumExamined',  9, ...                           % How many sizes to use for sampling the psychometric curve
        'maxLetterSizeDegs', 0.1, ...                               % The maximum letter size in degrees of visual angle
        ...
        'visualizedPSFwavelengths', [], ...                         % Vector with wavelengths for visualizing the PSF. If set to empty[] there is no visualization.
    ...
    subdir = 'Z:\ISETBioJandJ-main\data\';
    [fileList,~]=F_walkFolders1([subdir params.psfDataSubDir],'.mat');
    fileList = cellstr(fileList);
    subdir = [subdir params.psfDataSubDir];
    examinedPSFDataFiles = extractAfter(fileList,subdir);
    examinedPSFDataFiles = examinedPSFDataFiles';
